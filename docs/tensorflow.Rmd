...
```
Finally, this graph definition does can not be interfaced until a TF session is launched, and the TF variables we just created are initialized.
```{r}
sess <- tf$Session()
