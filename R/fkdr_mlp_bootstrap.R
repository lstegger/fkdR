...
layer2 = tf$add(tf$matmul(layer1, weight_variable(shape(n_hidden_1, n_hidden_2))), bias_variable(shape(n_hidden_2)))
layer2 = tf$nn$relu(layer2)

out_layer = tf$add(tf$matmul(layer2, weight_variable(shape(n_hidden_2, n_classes))), bias_variable(shape(n_classes)))
...
for(epoch in seq_len(training_epochs)) {
  for(batchNr in seq_len(numberOfBatches)) {
    rowIndices = sample(nrow(train.x), replace = TRUE)

    if(batchNr %% 10 == 0) {
      train_accuracy_train <- sess$run(accuracy, feed_dict = dict(x = train.x[rowIndices, ], y = train.y[rowIndices, ]))
      train_accuracy_test <- sess$run(accuracy, feed_dict = dict(x = train.x[-rowIndices, ], y = train.y[-rowIndices, ]))
      cat(sprintf("Epoch: %d | Batch: %d/%d | Training RMSE: %g | Test RMSE: %g\n", epoch, batchNr, numberOfBatches, train_accuracy_train, train_accuracy_test))
    }
